CLASS /dmo/tc_flight_travel_api11 DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC
  FOR TESTING
...
CLASS /dmo/tc_flight_travel_api11 IMPLEMENTATION.
  METHOD class_setup.
  ...
  METHOD cuerd_travel.
    DATA ls_travel_in  TYPE /dmo/if_flight_legacy11=>ts_travel_in.
    DATA ls_travel_inx TYPE /dmo/if_flight_legacy11=>ts_travel_inx.
    DATA ls_travel     TYPE /dmo/travel11.
    ...
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_CREATE11'
      EXPORTING
    ...
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_SAVE11'.

    " DB Check
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel11 WHERE travel_id = @lv_travel_id INTO ( @DATA(lv_agency_id), @DATA(lv_customer_id), @DATA(lv_description) ).
    ...
    " Action
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_SET_BOOK_11'
      EXPORTING
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV /dmo/travel_status11( /dmo/if_flight_legacy11=>travel_status-new ) ).
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV /dmo/travel_status11( /dmo/if_flight_legacy11=>travel_status-booked ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel-description  exp = 'My New Test' ).

    " Delete
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_DELETE11'
      EXPORTING
        iv_travel_id = lv_travel_id
      IMPORTING
        et_messages  = lt_messages.
