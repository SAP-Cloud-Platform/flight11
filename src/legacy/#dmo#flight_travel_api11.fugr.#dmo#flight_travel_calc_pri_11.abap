...
FUNCTION /DMO/FLIGHT_TRAVEL_CALC_PRI_11.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_FLIGHT) TYPE  /DMO/IF_FLIGHT_LEGACY11=>TT_FLIGHT
*"       OPTIONAL
*"----------------------------------------------------------------------
  TYPES BEGIN OF ty_seats_occupied.
  INCLUDE TYPE /dmo/flight11.
  TYPES   distance       TYPE /dmo/connecti_11-distance.
  TYPES   distance_unit  TYPE /dmo/connecti_11-distance_unit.
  ...
  IF it_flight IS SUPPLIED.
    SELECT FROM /dmo/booking11 AS booking
      JOIN     /dmo/connecti_11 AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     /dmo/flight11 AS flight
